...
class TestValidateCommand:
    def test_validate_valid_algorithm(self, valid_algo_dir):
        runner = CliRunner()
        result = runner.invoke(quilt, ["validate", "--path", str(valid_algo_dir)], catch_exceptions=False)
        assert result.exit_code == 0
        assert "valid" in result.output.lower() or "pass" in result.output.lower()

    def test_validate_missing_manifest(self, tmp_path):
        runner = CliRunner()
        result = runner.invoke(quilt, ["validate", "--path", str(tmp_path)])
        assert result.exit_code != 0
    ...
    def test_validate_scraper_skips_class_check(self, valid_scraper_dir):
        runner = CliRunner()
        result = runner.invoke(quilt, ["validate", "--path", str(valid_scraper_dir)], catch_exceptions=False)
        assert result.exit_code == 0
