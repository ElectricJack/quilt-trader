...
- **[IN SCOPE of integration spec — planned implementation]** (2026-05-28): Promoted from Tier 3 deferred → in-scope as P3 in `2026-05-28-backtest-and-validation-lab-integration.md` § Planned additions. Design: two-pass execution. Pass 1 (`on_start` + one warmup `on_tick`) discovers which symbols the algo uses via `ctx._bars` inspection. Pass 2 replays from bar 0 against `union(timestamps from each (src, symbol, tf) the algo touched)`. Simplifies invariant I3: the resolution layer is no longer needed inside the tick loop because the clock bar IS the symbol bar by construction. `_lookup_symbol_close` and `_try_fill` fill-bar lookup both reduce to direct dict access.
